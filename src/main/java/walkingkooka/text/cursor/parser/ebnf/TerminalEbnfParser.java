 ...
 */
final class TerminalEbnfParser implements Parser<EbnfParserContext>,
        RequiredParser<EbnfParserContext> {

    /**
     * Singleton instance
     */
    final static Parser<EbnfParserContext> INSTANCE = new TerminalEbnfParser().cast();

    private TerminalEbnfParser() {
        super();
    }

    @Override
    public Optional<ParserToken> parse(final TextCursor cursor, final EbnfParserContext context) {
        TerminalEbnfParserToken result = null;
        ...
        for (; ; ) {
            ...
            for (; ; ) {
                ...
                if (backslash) {
                    switch (c) {
                        ...
                        case 'u':
                            ...
                            break;
                        default:
                            throw new TerminalEbnfParserException("Invalid backslash sequence '" + c + "'");
                    }
                    backslash = false;
                    continue;
                }
                if (unicodeIndex >= 0) {
                    ...
                    if (-1 == digit) {
                        throw new TerminalEbnfParserException("Invalid unicode sequence '" + c + "'");
                    }
                ...
                if (open == c) {
                    result = TerminalEbnfParserToken.with(
                            raw.toString(),
